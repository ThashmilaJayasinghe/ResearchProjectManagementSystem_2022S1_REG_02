...
module.exports.getAllStaff = asyncHandler(async (req, res) => {
	Staff.find()
		.then((Staff) => {
			res.json(Staff);
		})
...
const addQualifications = asyncHandler(async (req, res) => {
	...
	try {
		const updateResult = await Staff.findOneAndUpdate(
			{ email: email },
			{ $push: { qualifications: newQualification } }
		);
...
const addResearchField = asyncHandler(async (req, res) => {
	...
	try {
		const updateResult = await Staff.findOneAndUpdate(
			{ email: email },
			{ $push: { researchInterests: newResearchFeild } }
		);
		res.status(200).json(updateResult);
	} catch (err) {
		console.log(err);
...
const removeQualifications = asyncHandler(async (req, res) => {
	...
	try {
		const deleteVal = await Staff.findOneAndUpdate(
			{ email: email },
			{
